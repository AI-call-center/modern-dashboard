      ...
      {isOpen && (
        ...
        >
          <motion.div
            ...
            className="relative w-full max-w-2xl p-8 bg-gray-900/95 rounded-2xl shadow-2xl border border-gray-800"
          >
            {step === 1 ? (
              <div className="text-center">
                ...
                <div className="mb-8">
                  <input
                    ...
                    className="w-full px-4 py-3 rounded-lg bg-gray-800 border border-gray-700 text-white focus:ring-2 focus:ring-cyan-500 focus:border-cyan-500 placeholder-gray-500"
                  />
                  {voiceName === '' && (
                ...
                <div className="flex justify-center space-x-8">
                  <motion.button
                    ...
                    className={`flex flex-col items-center p-6 rounded-xl border transition-colors group ${voiceName ? 'bg-cyan-600/20 border-cyan-500/30 hover:bg-cyan-600/30' : 'bg-gray-800 border-gray-700 cursor-not-allowed'}`}
                  >
                    <MicrophoneIcon className="w-12 h-12 text-cyan-400 group-hover:text-cyan-300" />
                    <span className="mt-4 text-cyan-400 group-hover:text-cyan-300">Record Your Voice</span>
                  </motion.button>
                  <label>
                    <motion.div
                      whileHover={{ scale: 1.05 }}
                      whileTap={{ scale: 0.95 }}
                      className={`flex flex-col items-center p-6 rounded-xl border transition-colors cursor-pointer group ${voiceName ? 'bg-purple-600/20 border-purple-500/30 hover:bg-purple-600/30' : 'bg-gray-800 border-gray-700 cursor-not-allowed'}`}
                    >
                      <CloudArrowUpIcon className="w-12 h-12 text-purple-400 group-hover:text-purple-300" />
                      <span className="mt-4 text-purple-400 group-hover:text-purple-300">Upload Audio File</span>
                    </motion.div>
                    <input
                      type="file"
                      accept="audio/*"
