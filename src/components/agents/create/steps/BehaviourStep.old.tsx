...
export default function BehaviourStep({ data, onChange }: BehaviourStepProps) {
  const [isGreetingTemplateModalOpen, setIsGreetingTemplateModalOpen] = useState(false);
  const [isPromptTemplateModalOpen, setIsPromptTemplateModalOpen] = useState(false);

  const handleGreetingTemplateClick = () => setIsGreetingTemplateModalOpen(true);
  const handlePromptTemplateClick = () => setIsPromptTemplateModalOpen(true);
  const addVariable = () => {
    onChange({
  ...
  return (
    <div className="space-y-8">
      ...
      {/* Template Modals */}
      <TemplateModal
        isOpen={isGreetingTemplateModalOpen}
        onClose={() => setIsGreetingTemplateModalOpen(false)}
        onSelectTemplate={(template) => {
          onChange({ greeting: template.description });
        }}
      />
      <TemplateModal
        ...
        onSelectTemplate={(template) => {
          onChange({ prompt: template.description });
        }}
      />
    </div>
      ...
      <TemplateModal
        ...
        onSelectTemplate={(template) => {
          onChange({ greeting: template.description });
          setIsGreetingTemplateModalOpen(false);
        }}
      />
      <TemplateModal
        isOpen={isPromptTemplateModalOpen}
        ...
        onSelectTemplate={(template) => {
          onChange({ prompt: template.description });
          setIsPromptTemplateModalOpen(false);
        }}
      />
    </div>
  );
